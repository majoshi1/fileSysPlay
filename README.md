...
An application can have large number of smaller-files and folders. Particularly, in case of frameworks and micro-architectures, there are large number of boiler-plate files. Even with modern editors like VS Code, it can be tedious to open each file to view or modify. With powerful source control systems, like Git, it is easier to track changes in large files. Also, searching text is already powerful in editors. So, it may make sense to store such applications in single large file.
...
Each generator creates a bare-minimum application, e.g. V1 of app1. But, it is not directly useful, without making additional changes, to create a usable application, kind V2 of the app1. These same changes may have to be done again and again for every new future applications e.g. app2, app3 etc. Enhancing generators to create the version V2 _directly_, is either not possible or not very easy. This results in repeated work of applying boiler-plate changes, for every new application. Also, by that time, app1 may already have non-reusable, business-specific changes as part of say version V3. So, copying the app1 folder as app2 is also not feasible. 

### Solution
The `scaffold-any-app` utility tries to fill-in this gap, by storing entire application in a tree-like structure, inside a single file. So it can be used to bootstrap new applications easily in future. Also, single file can be more easily kept up-to-date, as needed.

This is achieved by following approach:
...
Try to keep file and folder names and content generic, so that they can be re-used. Add any non-reusable files / folders in .gitignore file, so that they are skipped during generation of single-scaffold file.
...
### Binary files
Use base64 encoding for storing binary files. Also, it may be easier to just use Create mode to handle binary files.
